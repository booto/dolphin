...
void CommonAsmRoutines::GenConvertDoubleToSingle()
{
  ...
  // Clobbers RSCRATCH/RSCRATCH2
  // saves/restores XMM2 on stack

  const void* start = GetCodePtr();
  MOV(64, R(RSCRATCH2), R(RSP));
  SUB(64, R(RSP), Imm8(16));
  MOV(64, R(RSCRATCH), Imm64(0xfffffffffffffff0));
  AND(64, R(RSP), R(RSCRATCH));
  MOVAPD(MatR(RSP), XMM2);
  MOVAPD(XMM2, R(XMM0));
  // Grab Exponent
  PAND(XMM0, MConst(double_exponent));
  PSRLQ(XMM0, 52);
  ...
  // OR the sign bit in.
  MOVAPD(XMM1, R(XMM2));
  PAND(XMM1, MConst(double_sign_bit));
  PSRLQ(XMM1, 32);

  FixupBranch end = J(false); 
  ...
  // We want bits 0, 1
  MOVAPD(XMM0, R(XMM2));
  PAND(XMM0, MConst(double_top_two_bits));
  PSRLQ(XMM0, 32);

  // And 5 through to 34
  MOVAPD(XMM1, R(XMM2));
  PAND(XMM1, MConst(double_bottom_bits));
  PSRLQ(XMM1, 29);
  ...
  // OR them togther
  POR(XMM0, R(XMM1));
  MOVAPD(XMM2, MatR(RSP));
  MOV(64, R(RSP), R(RSCRATCH2));
  // End
  RET();
